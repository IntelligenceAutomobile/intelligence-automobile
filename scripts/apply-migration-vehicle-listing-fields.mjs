...
const sqlPath = resolve(
  __dirname,
  "../prisma/migrations/20260616120000_add_vehicle_listing_fields/migration.sql"
);
const sql = readFileSync(sqlPath, "utf-8");

const statements = sql
  .split("\n")
  .filter((line) => !line.trim().startsWith("--")) // retire les lignes de commentaire
  .join("\n")
  .split(";")
  .map((s) => s.trim())
  .filter((s) => s.length > 0);
...
console.log("\n✅ Migration « vehicle listing fields » appliquée à Turso !");
client.close();
