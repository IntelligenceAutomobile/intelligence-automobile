// Applique les tables « Reprises » (Reprise + RepriseEvent).
// Tables nouvelles, sans lien avec l'existant : la migration est rétro-compatible,
// le code d'avant continue de tourner avec.
// À lancer AVANT de déployer le code qui les utilise, sinon la page Reprises tombe
// en production sur « no such table: Reprise ».
//
//   node scripts/apply-migration-reprises.mjs         → Turso (production)
//   node scripts/apply-migration-reprises.mjs --dev   → dev.db (poste local)
//
...
if (dev) {
  ...
  // Les antislashs de Windows sont retournés, l'adaptateur libsql attendant
  // une adresse « file: » à séparateurs avant.
...
const sqlPath = resolve(__dirname, "../prisma/migrations/20260731210000_add_reprises/migration.sql");
