...
export default async function AccueilV1Page() {
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-12">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 items-center">
              ...
              <div className="py-20 lg:pl-16">
                ...
                <div className="space-y-0">
                  {[
                    { n: "1", title: "Vous choisissez", desc: "Parmi notre stock ou via mandat d'import sur-mesure." },
                    { n: "2", title: "Nous gérons tout", desc: "Achat, transport, homologation, immatriculation française." },
                    { n: "3", title: "Vous recevez les clés", desc: "Livraison à domicile sous 2 à 3 semaines, véhicule prêt à conduire." },
                  ].map((step) => (
                    <div
                      key={step.n}
                      className="flex items-start gap-6 py-7"
          ...
          >
            {[
              ...
              >
                ...
                <div
                  className="absolute inset-0"
                  style={{ background: "linear-gradient(to top, #070F1E 45%, rgba(7,15,30,0.5) 100%)" }}
                />
        ...
        {derniersVehicules.length > 0 && (
          ...
          >
            <div className="max-w-7xl mx-auto px-6 lg:px-12 pt-24 pb-10">
              <div className="flex items-end justify-between mb-16">
                <div>
                  <p className="text-xs tracking-[0.35em] uppercase mb-4" style={{ color: "#6B9FEE" }}>Stock actuel</p>
                  <h2 className="font-black uppercase leading-none" style={{ fontSize: "clamp(2rem, 4.5vw, 3.8rem)", letterSpacing: "-0.025em" }}>
                    Derniers arrivages
                  </h2>
                </div>
                <Link href="/vehicules" className="hidden md:inline-flex items-center gap-2 text-xs tracking-widest uppercase group" style={{ color: "#6B9FEE" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-px" style={{ backgroundColor: "#1B3055" }}>
              {derniersVehicules.map((v) => {
                const images = JSON.parse(v.images) as string[];
                const img = images[0] ?? null;
                return (
                  <Link href={`/vehicules/${v.id}`} key={v.id} className="group block overflow-hidden" style={{ backgroundColor: "#070F1E" }}>
                    <div className="relative overflow-hidden" style={{ height: "260px" }}>
                      {img ? (
                        <img src={img} alt={`${v.make} ${v.model}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-xs tracking-widest uppercase" style={{ backgroundColor: "#0D1A2D", color: "#1B3055" }}>
                          Photo à venir
                        </div>
                      )}
                      <div className="absolute inset-0" style={{ background: "linear-gradient(to top, #070F1E 0%, transparent 55%)" }} />
                    </div>
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-2xl font-black" style={{ color: "#F0F5FF" }}>{v.price.toLocaleString("fr-FR")} €</span>
                        <span className="text-xs tracking-widest uppercase" style={{ color: "#6B9FEE" }}>Détail →</span>
                      </div>
                    </div>
                  </Link>
                );
            ...
            <div className="text-center py-6 md:hidden">
              <Link href="/vehicules" className="text-sm tracking-widest uppercase" style={{ color: "#6B9FEE" }}>Tout le stock →</Link>
            </div>
          </section>
        )}

        {/* ── CTA FINAL ── */}
        <section className="relative overflow-hidden border-t" style={{ borderColor: "#1B3055", backgroundColor: "#070F1E" }}>
          <div className="absolute inset-0">
            <img src="https://images.unsplash.com/photo-1563720223185-11003d516935?auto=format&fit=crop&w=1600&q=70" alt="" className="w-full h-full object-cover object-center" style={{ opacity: 0.07 }} />
          </div>
          <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 py-36 text-center">
            <p className="text-xs tracking-[0.35em] uppercase mb-6" style={{ color: "#6B9FEE" }}>Prêt à commencer ?</p>
            <h2 className="font-black uppercase leading-[0.92] mx-auto mb-12" style={{ fontSize: "clamp(2.8rem, 6.5vw, 5.5rem)", letterSpacing: "-0.025em", maxWidth: "900px" }}>
              Votre prochain véhicule<br /><span style={{ color: "#6B9FEE" }}>Vous Attend.</span>
            </h2>
            <div className="flex flex-wrap gap-4 justify-center">
              <Link href="/contact" className="px-10 py-4 text-sm font-semibold tracking-widest uppercase rounded-full border-2 transition-all duration-300" style={{ borderColor: "#F0F5FF", color: "#F0F5FF" }}>
                Prendre contact
              </Link>
              <Link href="/vehicules" className="px-10 py-4 text-sm font-semibold tracking-widest uppercase rounded-full transition-all duration-300" style={{ backgroundColor: "#6B9FEE", color: "#070F1E" }}>
                Voir le stock
              </Link>
            </div>
          </div>
