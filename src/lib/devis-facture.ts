// Conversion d'un devis en facture (acompte / solde / complète).
// Extraite de la route /api/admin/devis/[id]/facture pour servir aussi la
// création directe depuis le stock : les contrôles légaux, la numérotation
// continue et la réservation du véhicule restent définis à un seul endroit.
import { prisma } from "./prisma";
import { reserveVehicleForQuote } from "./devis-effets";
import {
  computeTotals, factureNumber, facturePrefix, FACTURE_KIND_LABEL, quoteIssues, issuesLabel,
  mergeVehicleBlock, type FactureKind, type QuoteItem, type TvaMode, type DepositMode, type QuoteKind,
} from "./devis";
import { parisDay } from "./vehicules";
import type { Quote } from "@/generated/prisma/client";

export type FactureOutcome =
  | { ok: true; id: string; number: string }
  | { ok: false; error: string; status: number };

// Réutilise le moteur devis : la facture est une ligne Quote (docType=facture).
export async function convertQuoteToFacture(src: Quote, kind: FactureKind): Promise<FactureOutcome> {
  // Un avoir passait ce contrôle : on pouvait en tirer une facture.
  if (src.docType === "facture") return { ok: false, error: "Ce document est déjà une facture.", status: 400 };
  if (src.docType === "avoir") return { ok: false, error: "Un avoir ne se convertit pas en facture.", status: 400 };
  ...
  if (manques.length > 0) {
    return { ok: false, error: `Ce devis attend encore ${issuesLabel(manques)}.`, status: 400 };
  }

  if (kind === "acompte" && totals.deposit <= 0) {
    return { ok: false, error: "Ce devis ne prévoit pas d'acompte.", status: 400 };
  }
  ...
  if (deja) {
    return { ok: false, error: `${FACTURE_KIND_LABEL[kind]} ${deja.number} existe déjà pour ce devis.`, status: 409 };
  }
  ...
  return { ok: true, id: facture.id, number: facture.number };
}
